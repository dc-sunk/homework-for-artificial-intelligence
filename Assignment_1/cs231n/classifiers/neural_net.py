...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        activate_layer_1 = np.maximum(layer_1, 0)
        scores = np.dot(activate_layer_1, W2) + b2
        ...
        loss = np.log(np.exp(scores).sum(axis = 1)).sum() - scores[range(N), list(y)].sum()
        ...
        dscore[range(N), y] -= 1
        dscore /= N
        grads['W2'] = np.dot(activate_layer_1.T, dscore) + reg * W2
